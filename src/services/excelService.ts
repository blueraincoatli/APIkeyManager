...
export async function parseExcelFile(
  ...
  console.log(
    "parseExcelFile called with:",
    typeof filePath === "string" ? filePath : filePath.name,
  );
  ...
  try {
    ...
    const requiredColumns = [
      ...
      {
        key: "platform",
        headers: [currentHeaders.platform, "提供商", "Platform"],
      },
      {
        key: "description",
        headers: [currentHeaders.description, "描述", "Description"],
      },
    ];
    ...
    for (const col of requiredColumns) {
      ...
      for (const header of col.headers) {
        index = headers.findIndex((h) => h && h.toString().trim() === header);
        if (index !== -1) break;
      }

      if (index === -1) {
        console.error(
          `Missing column: ${col.key}. Expected headers: ${col.headers.join(", ")}. Available headers:`,
          headers,
        );
        return {
          success: false,
    ...
    for (let i = 1; i < jsonData.length; i++) {
      ...
      if (!name || !keyValue) {
        console.warn(
          `第${i + 1}行数据不完整，跳过: 名称=${name}, API Key=${keyValue}`,
        );
        continue;
      }
...
export function generateExcelTemplate(): Uint8Array {
  ...
  const templateData = [
    [
      currentHeaders.name,
      currentHeaders.apiKey,
      currentHeaders.platform,
      currentHeaders.description,
    ],
    [
      "OpenAI GPT-4",
    ...
    [
      "Claude API",
      "claude-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
      "Anthropic",
      "用于Claude模型访问",
    ],
    [
      "Google Gemini",
      "AIzaSyxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
      "Google",
      "用于Gemini模型访问",
    ],
  ];
