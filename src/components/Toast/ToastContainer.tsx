...
const Toast: React.FC<ToastProps> = ({ toast, onRemove }) => {
  ...
  const getToastStyles = () => {
    const baseStyles =
      "transform transition-all ease-in-out translate-x-0 opacity-100";
    const exitStyles = "transform translate-x-full opacity-0";

    // 使用项目的渐变色效果，与面板保持一致
    const typeStyles = {
      success:
        "bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/40 dark:to-green-800/30 border-green-200 dark:border-green-700/50",
      error:
        "bg-gradient-to-br from-red-50 to-red-100 dark:from-red-900/40 dark:to-red-800/30 border-red-200 dark:border-red-700/50",
      warning:
        "bg-gradient-to-br from-yellow-50 to-yellow-100 dark:from-yellow-900/40 dark:to-yellow-800/30 border-yellow-200 dark:border-yellow-700/50",
  ...
  const getIcon = () => {
    ...
    > = {
      success: (
        <svg
          className={cn(iconStyles, iconColors.success)}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path
      ...
      error: (
        <svg
          className={cn(iconStyles, iconColors.error)}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path
      ...
      warning: (
        <svg
          className={cn(iconStyles, iconColors.warning)}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path
      ...
      info: (
        <svg
          className={cn(iconStyles, iconColors.info)}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path
  ...
  const ariaLiveValue =
    toast.type === "error" ? ("assertive" as const) : ("polite" as const);
